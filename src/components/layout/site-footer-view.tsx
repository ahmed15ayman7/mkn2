...
export function SiteFooterView({ labels, isRtl, year }: Props) {
  const Chevron = isRtl ? ChevronLeft : ChevronRight;

  return (
    <footer className="relative text-white">
      <FooterBackground isRtl={isRtl} />
      <ScrollSection
        preset="site-footer"
        as="div"
      ...
      >
        <StaggerChildren
          preset="site-footer"
          isRtl={isRtl}
          className="grid gap-12 md:grid-cols-2 lg:grid-cols-4 lg:gap-0"
        >
          <FooterColumn withDivider className="md:ps-0" columnIndex={0} isRtl={isRtl}>
          ...
          <FooterColumn columnIndex={1} isRtl={isRtl}>
            <p className="max-w-sm text-sm leading-relaxed text-white/90">
              {labels.about}
            </p>
          </FooterColumn>

          <FooterColumn columnIndex={2} isRtl={isRtl}>
          ...
          <FooterColumn withDivider={false} className="md:pe-0" columnIndex={3} isRtl={isRtl}>
            <h3 className="text-sm font-semibold text-brand-gold">
              {labels.office}
            </h3>
            <ul className="mt-5 space-y-4 text-sm leading-relaxed text-white/90">
              ...
              <li className="flex gap-3">
                <Phone
                  ...
                  strokeWidth={1.75}
                />
                <span dir="ltr">{labels.phones}</span>
              </li>
            </ul>
      ...
      <FooterCopyrightBar
        ...
        designedBy={labels.designedBy}
      />
    </footer>
  );
}

function FooterBackground({ isRtl }: { isRtl: boolean }) {
  return (
    <div className="pointer-events-none absolute inset-0">
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(to ${!isRtl ? "right" : "left"}, rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.25))`,
        }}
        aria-hidden
      />
    </div>
